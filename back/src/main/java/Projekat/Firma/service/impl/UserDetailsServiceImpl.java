...
public class UserDetailsServiceImpl implements UserDetailsService {
  ...
  public String getKorisnikId(UserDetails userDetails) {
      String username = userDetails.getUsername();
      // Assuming korisnikService can fetch korisnikId based on username
      Korisnik korisnik = korisnikService.findbyKorisnickoIme(username).orElse(null);
      if (korisnik != null) {
          return korisnik.getId().toString();
      } else {
          // Return a default value or handle the case when korisnik is not found
          return "unknown";
      }
  }
  ...
  public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
    ...
    if (korisnik == null) {
      throw new UsernameNotFoundException(String.format("No user found with username '%s'.", username));
        ...
        String korisnikId  = korisnik.getId().toString();
        String imePrezime = korisnik.getIme() + 
